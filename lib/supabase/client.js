...
function missingSupabaseClient() {
  ...
  return {
    ...
    auth: {
      ...
      signOut: async () => { window.location.href = "/"; return { error: null }; },
...
export function createClient() {
  ...
  // If keys are present but we want to force demo, use DEMO flag.
  // If keys are missing (common in preview), default to mock client automatically.
  if (url && anonKey && !DEMO) {
...
export function getDemoLessonCatalog() {
  // Always return something for safety in recommendations
  return [
